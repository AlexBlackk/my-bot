...
def kb_after_order():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ Vreau o ofertă",      callback_data="oferta")],
        [InlineKeyboardButton("📩 A comanda serviciul", callback_data="comanda")],
        [InlineKeyboardButton("🎬 Portofoliu",          callback_data="portofoliu")],
    ])
...
async def entry_oferta(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Кнопка '✏️ Vreau o ofertă' на экране тарифов — сразу описание проекта."""
    query = update.callback_query
    await query.answer()
    ctx.user_data["service"] = "altceva"
    ctx.user_data["project"] = ""
    ctx.user_data["extra"] = ""
    await query.message.reply_text(DESCRIBE_TEXTS["altceva"])
    return DESCRIBING
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(entry_oferta, pattern="^oferta$"),
        ],
        states={
            STARTED: [
